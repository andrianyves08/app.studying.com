<?php
    ...
    public function get_all($limit = FALSE, $row = FALSE, $id){
    	...
    	$this->db->join('products_modules', 'products_modules.course_ID = course.id');
    	$this->db->join('users_products', 'products_modules.product_ID = users_products.product_ID');
    ...
    public function search_course($slug, $search, $id){
    	...
    	');
    	$this->db->join('products_modules', 'products_modules.course_ID = course.id');
    	$this->db->join('products', 'products_modules.product_ID = products.id');
    	$this->db->join('users_products', 'products_modules.product_ID = users_products.product_ID');
    	$this->db->join('course_section', 'course.id = course_section.course_ID');
		$this->db->where('users_products.user_ID', $id);
		if($slug != NULL){
	...
	public function create_section($section, $course_id, $course_slug){
		...
		$this->db->cache_delete('modules', $course_slug);
		...
		$this->db->cache_delete('chech-purchases', $course_slug);
		if ($this->db->trans_status() === FALSE){
		    $this->db->trans_rollback();
		    return false;
		} else{
